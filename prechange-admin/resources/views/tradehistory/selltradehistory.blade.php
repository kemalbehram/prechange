...
<section class="content">
	<div class="card">
		<div class="card-body conts">
			<div class="row">
				<div class="col-md-12">
					<h5 class="text-uppercase maginf">Sell Trade History</h5>
				</div>
			</div>
		</div>
	</div>
			...
			<div class="row">
				<div class="col-md-6 tg-select-left">
					<select class="form-control custom-s-left" onchange="pageredirect(this)">
                        <option value="{{ url('admin/sell_tradehistory/'.$tradepair->coinone.'_'.$tradepair->cointwo.'/limit') }}" @if($order_type==1) selected @endif>Limit</option>
                        <option value="{{ url('admin/sell_tradehistory/'.$tradepair->coinone.'_'.$tradepair->cointwo.'/market') }}" @if($order_type==2) selected @endif>Market</option>
                    </select>
				</div>
				<div class="col-md-6 tg-select">
                    ...
                    @if(isset($pairs))
                        <option value="{{ url('admin/sell_tradehistory/'.$tradepair->coinone.'_'.$tradepair->cointwo.'/limit') }}">{{ $tradepair->coinone }} / {{ $tradepair->cointwo }}</option>
                        @foreach($pairs as $coinones) 
	                        @if($coinones->coinone.'_'.$coinones->cointwo != $tradepair->coinone.'_'.$tradepair->cointwo)
	                        	<option value="{{ url('admin/sell_tradehistory/'.$coinones->coinone.'_'.$coinones->cointwo.'/limit') }}">{{ $coinones->coinone }} / {{ $coinones->cointwo }}</option>
	                        @endif
                        @endforeach
                    @endif
                  </select>
						...
						<table class="table" id="dows">
							...
							<tbody>
								...
								<tr>
									...
									<td>{{ number_format($price = markertSellPrice($trade->id)->price, 8, '.', '') }}</td>
									@else
